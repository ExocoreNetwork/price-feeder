...
func RunPriceFeeder(conf feedertypes.Config, logger feedertypes.LoggerInf, mnemonic string, sourcesPath string, standalone bool) {
	...
	// we don't handle empty tokenfeeders list
	fsMap := make(map[int]*Feeder)
	for feederID, feeder := range oracleP.TokenFeeders {
		...
		if strings.EqualFold(tokenName, nstToken) {
			source = fetchertypes.BeaconChain
		}
		fsMap[feederID] = NewFeeder(feeder, feederID, f, ecClient, source, tokenName, 0, feedertypes.GetLogger(fmt.Sprintf("feed_%s", tokenName)))
	}

	feeders := NewFeeders(fsMap)
	feeders.Start()

	for event := range ecClient.EventsCh() {
		switch event.Type {
		case exoclient.NewBlock:
		case exoclient.UpdatePrice:
		case exoclient.UpdateNST:
		}
	}
	...
	// 	if strings.EqualFold(oracleP.Tokens[feeder.TokenID].Name, fetchertypes.NativeTokenETH) {
	// 		tokenName = strings.ToLower(oracleP.Tokens[feeder.TokenID].Name)
	// 	} else {
	...
	// 	// TODO: refactor
	// 	if strings.EqualFold(tokenName, fetchertypes.NativeTokenETH) {
	...
	// 		if success := f.UpdateNativeTokenValidators(fetchertypes.NativeTokenETH, r.NativeETH); !success {
	// 			stakerInfos, err := exoclient.GetStakerInfos(cc, fetchertypes.NativeTokenETHAssetID)
	// 			for err != nil {
	// 				logger.Error("Failed to get stakerInfos, retrying...")
	// 				stakerInfos, err = exoclient.GetStakerInfos(cc, fetchertypes.NativeTokenETHAssetID)
	// 				time.Sleep(2 * time.Second)
	// 			}
	// 			f.ResetStakerValidatorsForAll(fetchertypes.NativeTokenETH, stakerInfos)
	// 		}
	// 	}
...
//
//	func feedToken(fInfo *feederInfo, f *fetcher.Fetcher, conf types.Config) {
//		logger := getLogger()
//		pChan := make(chan *fetchertypes.PriceInfo)
//		prevPrice := ""
//		prevDecimal := -1
//		prevHeight := uint64(0)
...
//				if strings.EqualFold(fInfo.params.tokenName, fetchertypes.NativeTokenETH) {
...
//
//				if !(fInfo.params.tokenName == fetchertypes.NativeTokenETH) {
